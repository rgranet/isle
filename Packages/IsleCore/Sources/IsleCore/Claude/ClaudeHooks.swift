...
public enum ClaudePermissionUpdate: Equatable, Codable, Sendable {
    ...
    public var displayLabel: String {
        ...
        case let .setMode(_, mode):
            switch mode {
            case .acceptEdits:
                // Matches Claude Code's own prompt option text — auto-accept
                // every edit for the rest of this session.
                return "Yes, allow all edits during this session"
            case .bypassPermissions, .dontAsk:
                return "Yes, bypass all permissions this session"
            case .plan:
                return "Switch to Plan mode"
            case .default:
                return "Switch to Default mode (ask each time)"
            case .auto:
...
public extension ClaudeHookPayload {
    ...
    var permissionRequestTitle: String {
        // Match Claude Code's own question style as closely as we can.
        // We synthesise the question from the tool name + the affected path
        // (when known) so the in-island prompt reads like Claude's terminal
        // prompt the user would otherwise see.
        let pathHint: String? = {
            if let preview = toolInputPreview, !preview.isEmpty {
                let trimmed = preview
                    .split(separator: "\n").first.map(String.init) ?? preview
                return trimmed.count > 60
                    ? String(trimmed.prefix(57)) + "…"
                    : trimmed
            }
            if let path = extractedPathValue, !path.isEmpty {
        ...
        case "Write":
            return pathHint.map { "Do you want to create this file at \($0)?" } ?? "Do you want to write this file?"
        case "Edit", "MultiEdit":
            return pathHint.map { "Do you want to make this edit to \($0)?" } ?? "Do you want to make this edit?"
        case "Bash":
            return pathHint.map { "Do you want to run this command?  \($0)" } ?? "Do you want to run this command?"
        case "Read":
            return pathHint.map { "Allow Claude to read \($0)?" } ?? "Allow Claude to read this file?"
        case "Glob", "Grep":
            return pathHint.map { "Allow Claude to search \($0)?" } ?? "Allow Claude to search?"
        case let toolName? where toolName.hasPrefix("mcp__"):
            return "Allow MCP tool \(toolName.dropFirst(5))?"
        case let toolName?:
