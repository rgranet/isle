...
public final class BridgeServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.withmii.isle.bridge.server")
    ...
    /// Isle M7.8 — fires every time an `AgentEvent` is applied to
    /// `localState`. The callback receives the freshly-merged sessions
    /// array so observers can publish to SwiftUI without owning a copy
    /// of the IPC code.
    ///
    /// Invoked on the BridgeServer's internal serial queue (NOT main).
    /// Observers MUST hop to their own actor before mutating UI state.
    public var onSessionsChanged: (@Sendable ([AgentSession]) -> Void)?
        ...
        case let .resolvePermission(sessionID, resolution):
            ...
            if let interaction = pendingCursorInteractions.removeValue(forKey: sessionID) {
                ...
                case let .deny(message, _):
                    ...
                    summary = message ?? "Permission denied in Isle."
                    phase = .completed
                }
            ...
            broadcast([.event(
                resolution.isApproved
                    ...
                    : .sessionCompleted(
                        SessionCompleted(
                            sessionID: sessionID,
                            summary: "Permission denied in Isle.",
                            timestamp: .now
                        )
                    )
            )])
                    ...
                    .permissionRequested(
                        PermissionRequested(
                            ...
                            request: PermissionRequest(
                                ...
                                // Match Claude Code's own terminal prompt:
                                // "1. Yes / 2. <suggestion> / 3. No".
    ...
    private func clearStaleCursorInteractionIfNeeded(for sessionID: String) {
        ...
        emit(
            .actionableStateResolved(
                ActionableStateResolved(
                    sessionID: sessionID,
                    summary: "Approval was handled outside Isle.",
                    timestamp: .now
                )
            )
        )
    ...
    private func resolvePendingOpenCodeInteraction(
        ...
        case let (.permission(_), .deny(message, _)):
            directive = .deny(reason: message ?? "Permission denied in Isle.")
            summary = message ?? "Permission denied in Isle."
            phase = .completed
    ...
    private func resolvePendingApproval(sessionID: String, approved: Bool) {
        ...
        if approved {
            ...
            response = .codexHookDirective(.deny(reason: "Permission denied in Isle."))
        }
    ...
    private func resolvePendingClaudeInteraction(
        ...
        case let (.permission(_), .deny(message, interrupt)):
            directive = .permissionRequest(
                .deny(message: message ?? "Permission denied in Isle.", interrupt: interrupt)
            )
            summary = message ?? "Permission denied in Isle."
            phase = .completed
    ...
    private func emit(_ event: AgentEvent) {
        ...
        onSessionsChanged?(localState.sessions)
    }

    /// Isle M7.9 — public entry point for the app to resolve a pending
    /// permission directly (without going through a fake socket round-trip).
    ///
    /// Routes to the same internal handler clients use over the socket so
    /// every pending-interaction branch (Claude, OpenCode, Cursor, …) is
    /// covered without duplicating logic. The internal resolve paths
    /// mutate `localState` directly without flowing through `emit`, so
    /// we explicitly notify subscribers after `handle` returns or the
    /// UI would never clear the "waiting for approval" state.
    public func resolvePermission(sessionID: String, resolution: PermissionResolution) {
        queue.async { [weak self] in
            guard let self else { return }
            self.handle(
                .resolvePermission(sessionID: sessionID, resolution: resolution),
                from: UUID()
            )
            self.onSessionsChanged?(self.localState.sessions)
        }
    }
