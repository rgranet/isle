...
extension Defaults.Keys {
    ...
    /// 0 = fully glassy bottom, 1 = opaque black (glass disabled visually).
    static let notchLiquidGlassDimming = Key<Double>("notchLiquidGlassDimming", default: 0.2)
    ...
    static let lockScreenWeatherProviderSource = Key<LockScreenWeatherProviderSource>("lockScreenWeatherProviderSource", default: .appleWeatherKit)
    ...
    // MARK: Coding Agents (Isle)
    /// Master toggle for the Isle coding-agent integration (Claude Code / Codex / OpenCode / …).
    static let enableCodingAgents = Key<Bool>("enableCodingAgents", default: true)
    static let enableNotchWeather = Key<Bool>("enableNotchWeather", default: true)
    /// Seconds between transcript polls. Set to 5s by default — matches open-vibe-island.
    static let codingAgentsPollingInterval = Key<Double>("codingAgentsPollingInterval", default: 5.0)
    /// Show a colored dot in the closed-notch right slot when at least one session is active.
    static let codingAgentsShowClosedNotchIndicator = Key<Bool>("codingAgentsShowClosedNotchIndicator", default: true)
    /// Play a discreet audio cue when a session needs user attention.
    static let codingAgentsSoundEnabled = Key<Bool>("codingAgentsSoundEnabled", default: true)
    /// Total seconds shown on the in-card permission-expiry progress bar.
    /// Claude's default PreToolUse hook timeout is 600s; we surface a more
    /// human number by default so the bar is meaningful — the actual hook
    /// timeout is governed by Claude itself.
    static let codingAgentsPermissionTimeoutSeconds = Key<Double>("codingAgentsPermissionTimeoutSeconds", default: 300.0)
    /// Cap on the in-memory permission history shown in the Agents tab.
    static let codingAgentsHistoryLimit = Key<Int>("codingAgentsHistoryLimit", default: 25)
    /// Briefly grow the notch (like the volume HUD) when a new permission
    /// request arrives. Compact indicator stays after the grow collapses.
    static let codingAgentsGrowNotchOnArrival = Key<Bool>("codingAgentsGrowNotchOnArrival", default: true)

    // MARK: Messaging apps (v1.1)

    /// Master switch for the messaging-app monitor (Teams / WhatsApp /
    /// Slack / iMessage). When off, the polling stops and the Messages
    /// tab disappears from the notch.
    static let enableMessagingApps = Key<Bool>("enableMessagingApps", default: true)
    /// Seconds between dock-badge AX polls.
    static let messagingPollingInterval = Key<Double>("messagingPollingInterval", default: 5.0)
    /// Whether the closed-notch slot lights up when any monitored app
    /// has unread messages.
    static let messagingShowClosedNotchIndicator = Key<Bool>("messagingShowClosedNotchIndicator", default: true)
    /// Per-app enable toggles. Keys are auto-generated from the
    /// MessagingApp raw value (e.g. `messagingApp_whatsapp`).
    static let messagingAppWhatsAppEnabled = Key<Bool>("messagingApp_whatsapp", default: true)
    static let messagingAppTeamsEnabled = Key<Bool>("messagingApp_teams", default: true)
    static let messagingAppSlackEnabled = Key<Bool>("messagingApp_slack", default: true)
    static let messagingAppIMessageEnabled = Key<Bool>("messagingApp_imessage", default: true)
    static let messagingAppDiscordEnabled = Key<Bool>("messagingApp_discord", default: true)
