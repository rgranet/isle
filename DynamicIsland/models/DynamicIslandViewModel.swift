...
class DynamicIslandViewModel: NSObject, ObservableObject {
    ...
    var effectiveClosedNotchHeight: CGFloat {
        ...
        // v1.2 — when an Isle arrival expansion is active (a new coding-
        // agent permission or a new unread message just landed), grow
        // the closed-notch height so the expanded bandeau has room to
        // breathe. Auto-collapses 3 s later via the existing
        // `expandingView` auto-hide timer.
        if coordinator.expandingView.show, coordinator.expandingView.type.isIsleArrivalExpansion {
            return max(closedNotchSize.height, Self.arrivalExpansionHeight)
        }
