...
class ScreenRecordingManager: ObservableObject {
    ...
    /// Bundle IDs of apps that legitimately watch the screen at all times
    /// without it being a "recording" the user wants flagged. DisplayLink and
    /// friends drive external monitors via screen capture, so `CGSIsScreenWatcherPresent()`
    /// always returns true on those setups. We treat the watcher as benign
    /// when one of these apps is running.
    private static let knownBenignScreenWatchers: Set<String> = [
        "com.displaylink.DisplayLinkUserAgent",  // DisplayLink Manager
        "com.synergy-foss.Synergy",              // Synergy (cross-machine input/screen)
        "com.kessel.Synergy",                    // Synergy (alt bundle)
        "com.kairos.duet",                       // Duet Display
        "com.kairos.duetcompanion",
        "com.duetdisplay.duetdisplay",
        "tv.spacedesk.spacedeskMacAgent",        // spacedesk
        "com.lunadisplay.luna",                  // Luna Display
    ]

    /// True when at least one known benign screen watcher is currently
    /// running. Cheap NSRunningApplication scan, called only when the
    /// private CG API says "someone is watching".
    private var isBenignScreenWatcherRunning: Bool {
        let running = NSWorkspace.shared.runningApplications
        return running.contains { app in
            guard let bundleID = app.bundleIdentifier else { return false }
            return Self.knownBenignScreenWatchers.contains(bundleID)
        }
    }

    /// Check current recording status using private API
    func checkRecordingStatus() {
        let watcherPresent = CGSIsScreenWatcherPresent()
        // Suppress the recording indicator when a known benign watcher
        // (DisplayLink, Duet, Synergy, …) is the only thing using screen
        // capture. Real screen recordings while one of these runs will
        // be missed — acceptable trade-off because the indicator would
        // otherwise be permanently lit and unusable.
        let currentRecordingState = watcherPresent && !isBenignScreenWatcherRunning

        // Debug: Always log current check
        print("ScreenRecordingManager: 🔍 Checking... current=\(isRecording), watcher=\(watcherPresent), benign=\(isBenignScreenWatcherRunning), effective=\(currentRecordingState)")
