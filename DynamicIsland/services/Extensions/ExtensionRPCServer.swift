...
final class ExtensionRPCServer {
    ...
    private let queue = DispatchQueue(label: "com.withmii.Isle.rpc.server", qos: .userInitiated)
    ...
    private func handleListenerState(_ state: NWListener.State) {
        switch state {
        case .ready:
            Logger.log("Started Isle RPC WebSocket server on port \(port)", category: .extensions)
