...
enum SneakContentType: Equatable {
    ...
    // Isle v1.2 — notch grow-on-arrival expansion types.
    // Pushed via `toggleExpandingView` when a new permission / unread
    // message arrives so the notch briefly grows (like the battery hover
    // HUD) before collapsing back to the compact indicator.
    case codingAgentArrived(brandColorHex: String, title: String)
    case messagingArrived(appID: String, count: Int)
}
...
extension SneakContentType {
    static func == (lhs: SneakContentType, rhs: SneakContentType) -> Bool {
        ...
        case let (.extensionLiveActivity(lb, la), .extensionLiveActivity(rb, ra)):
            return lb == rb && la == ra
        case let (.codingAgentArrived(lhex, ltitle), .codingAgentArrived(rhex, rtitle)):
            return lhex == rhex && ltitle == rtitle
        case let (.messagingArrived(lid, lc), .messagingArrived(rid, rc)):
...
extension SneakContentType {
    ...
    /// True for Isle's grow-on-arrival types. Used by the dispatch in
    /// ContentView to render an expanded bandeau and by
    /// `calculateDynamicNotchSize` to puff the notch height while active.
    var isIsleArrivalExpansion: Bool {
        switch self {
        case .codingAgentArrived, .messagingArrived: return true
...
class DynamicIslandViewCoordinator: ObservableObject {
    ...
    private static let tabOrder: [NotchViews] = [.home, .codingAgents, .messages, .shelf, .timer, .stats, .colorPicker, .notes, .clipboard, .terminal, .extensionExperience]
