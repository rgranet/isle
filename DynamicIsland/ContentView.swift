          ...
          VStack(alignment: .leading) {
              VStack(alignment: .leading) {
                  if coordinator.firstLaunch {
                      ...
                      // v1.2 — arrival expansion takes priority over the
                      // compact indicators. Auto-collapses after 3s via the
                      // existing `expandingView` auto-hide timer, then the
                      // compact attention/messaging indicators take over.
                      } else if vm.notchState == .closed,
                                coordinator.expandingView.show,
                                case let .codingAgentArrived(brandHex, title) = coordinator.expandingView.type,
                                !vm.hideOnClosed {
                          CodingAgentArrivedExpandedView(brandColorHex: brandHex, title: title)
                              .transition(
                                  .asymmetric(
                                      insertion: .scale(scale: 0.6, anchor: .top)
                                          .combined(with: .opacity)
                                          .animation(.spring(response: 0.48, dampingFraction: 0.7)),
                                      removal: .opacity.animation(.smooth(duration: 0.3))
                                  )
                              )
                      } else if vm.notchState == .closed,
                                coordinator.expandingView.show,
                                case let .messagingArrived(appID, count) = coordinator.expandingView.type,
                                !vm.hideOnClosed {
                          MessagingArrivedExpandedView(appID: appID, count: count)
                              .transition(
                                  .asymmetric(
                                      insertion: .scale(scale: 0.6, anchor: .top)
                                          .combined(with: .opacity)
