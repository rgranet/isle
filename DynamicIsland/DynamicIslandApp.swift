...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("Isle", image: "MenuBarIcon", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
//    private var windowSizeUpdateWorkItem: DispatchWorkItem?
    ...
    func applicationWillTerminate(_ notification: Notification) {
        let userInfo: [String: Any] = [
            IsleDistributedNotifications.UserInfoKey.sourcePID: NSNumber(value: ProcessInfo.processInfo.processIdentifier)
        ]
        DistributedNotificationCenter.default().postNotificationName(
            IsleDistributedNotifications.didBecomeIdle,
            object: nil,
            userInfo: userInfo,
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DistributedNotificationCenter.default().postNotificationName(
            IsleDistributedNotifications.didBecomeActive,
            object: nil,
            userInfo: userInfo,
            deliverImmediately: true
        )
        ...
        // Isle (M7.7): start the IPC server that receives hook events from the
        // IsleHooks CLI invoked by agent hooks. Failure is non-fatal — the
        // app keeps running, just without live hook events.
        if Defaults[.enableCodingAgents] {
            AgentHookBridgeManager.shared.startIfNeeded()
            // M7.9 popup-presenter is intentionally NOT started. Per user
            // feedback the permission UX lives entirely inside the notch
            // (inline Allow/Deny buttons on the highlighted session card),
            // no floating window. Leaving the presenter code in place as
            // an opt-in fallback for headless terminal sessions later.
            // AgentPermissionPresenter.shared.start()
        }
    ...
    private func installTopMenuItemsIfNeeded() {
        ...
        if mainMenu.items.contains(where: { $0.identifier?.rawValue == "Isle.Focus.Menu" }) {
        ...
        focusMenuItem.identifier = NSUserInterfaceItemIdentifier("Isle.Focus.Menu")
        ...
        accessibilityMenuItem.identifier = NSUserInterfaceItemIdentifier("Isle.Accessibility.Menu")
        ...
        permissionsMenuItem.identifier = NSUserInterfaceItemIdentifier("Isle.Permissions.Menu")
