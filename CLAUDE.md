...
- `DynamicIsland/extensions/View+SiriGlow.swift` (breathing glow modifier, shared by agent live activities)
- `DynamicIsland/helpers/NotchGlassSurface.swift` (frosted behind-window blur for the open notch)
- `DynamicIsland/extensions/View+EdgeFade.swift` (mask-based scroll-edge fades, replaces black paint-over gradients)
- `DynamicIsland/components/Notch/NotchDockView.swift` (floating tab dock below the open panel)
- `DynamicIsland/components/Tabs/NotchTabCatalog.swift` (shared tab-list builder for TabSelectionView + NotchDockView)
- `DynamicIsland/components/UI/SiriDotsSpinner.swift` (Siri-style glowing dots spinner)
...
- **Liquid glass notch surface (progressive).** The open notch renders `NotchGlassSurface` (an `NSVisualEffectView`, `.hudWindow` + `.behindWindow`, forced `vibrantDark`) under a **vertical gradient**: opaque black from the top to ~42 % of the height (content zone keeps full contrast), then melting to `Defaults[.notchLiquidGlassDimming]` black at the bottom edge so the blurred wallpaper glows through the rounded corners. A thin white rim (`notchGlassEdgeHighlight`, gradient stroke, clear at top) marks the bottom glass edge. See `ContentView.notchSurfaceBackground`, gated by `Defaults[.enableNotchLiquidGlass]`. **Do NOT use the private `NSGlassEffectView` (`LiquidGlassBackground`) for the notch**: inside the borderless notch window its backdrop doesn't blur at all — content behind shows through razor-sharp (tried, reverted). And do NOT flat-dim the whole panel (uniform tint over blur) — it reads as a muddy gray wash (also tried, reverted). `NotchGlassSurface` overrides `hitTest` to return nil so hover-open/click detection keeps working, shapes itself via `maskImage` (behind-window blurs ignore superlayer masks), and gets `padding(.top, -cornerRadius)` so its rounded top corners never carve into the clip silhouette. Closed notch stays opaque black (hardware blend). **Widget content must never paint opaque black over itself**: the upstream scroll-edge fades (black `LinearGradient` overlays on ScrollViews) rendered as ugly dark bands on the glass — they were all replaced with `fadedVerticalEdges` / `fadedHorizontalEdges` (`View+EdgeFade.swift`), which fade the content via a transparency mask instead. Any new scrollable notch widget must use those, never a painted gradient.
- **Floating dock geometry is reserved in `addShadowPadding`.** `NotchDockView` (gated by `Defaults[.enableNotchDock]`, hidden in minimalistic mode) floats below the open panel via `.overlay(alignment: .bottom).offset(y: notchDockGap + notchDockHeight)` in `ContentView.mainLayoutBase`. The vertical room for it comes from `notchDockReservedHeight(isMinimalistic:)` folded into `addShadowPadding` (`sizing/matters.swift`) — that one function feeds every window-sizing site (`calculateRequiredNotchSize`, `vm.open()`, etc.). **Critical: the dock reservation must NEVER be included in the height proposed to the panel** (`ContentView.rootContentMaxHeight` deliberately excludes it): greedy tab layouts (Weather fills whatever height it is offered) would grow into the dock strip and the dock would overlap the panel. The window is taller than the panel's proposal; the dock overlay hangs in that extra strip, with `dockDropOffset(panelHeight:)` (GeometryReader-measured) pulling it up as a last-resort clamp so it can never leave the window. Switching from a taller tab to a shorter one shrinks the panel under the cursor — `handleHover`'s exit branch uses an 800 ms close grace (vs 100 ms) within 1 s of a tab switch (`lastTabSwitchDate`) so the notch doesn't slam shut mid-navigation. Hover: the panel's interaction `.contentShape` is `notchInteractionShape` — the notch silhouette UNIONED with the dock strip (`NotchWithDockHoverShape`, a path extending below the view's bounds) while the dock is visible, so hovering the dock counts as hovering the notch and the auto-close never races the click. Do NOT try a separate `onHover` + auto-close-suppression on the dock instead: the 100 ms close timer fires before the dock hover registers and the notch slams shut. Tab list comes from `NotchTabCatalog.tabs()` (shared with `TabSelectionView`); the header hides `TabSelectionView` when the dock is enabled. The notch printer badge (`PrintLiveActivity`, fed by `PrintJobManager` polling every 2 s, gated by `Defaults[.enablePrintListener]`) reads the CUPS queue through `printing/IslePrintQueueReader.{h,m}` — `cupsGetJobs` for active jobs plus a per-job IPP `Get-Job-Attributes` request for `job-media-sheets[-completed]` (falls back to impressions). Swift never imports `<cups/cups.h>`; the ObjC `.m` does, and only Foundation types cross into Swift via the bridging header. **Linking needs `OTHER_LDFLAGS = -lcups`** in the app target (both Debug + Release build settings in the pbxproj) — without it you get undefined `cupsGetJobs`/`ippNewRequest` symbols at link time. CUPS APIs are Apple-"deprecated" but still the only supported queue reader, so the `.m` wraps everything in `#pragma clang diagnostic ignored "-Wdeprecated-declarations"`. The `1 of 1` page label only appears when the driver reports sheet counts; otherwise the badge shows with a spinner. The bridge does blocking IPP calls, so `PrintJobManager` polls off the main queue (`pollQueue`) and hops back to `@MainActor` to publish. App sandbox is OFF, so connecting to the local cupsd socket just works; if it's ever re-enabled, CUPS access will need a sandbox exception.
...
| 1.3.0 | **Liquid-glass design pass.** Progressive glass surface on the open notch (`NotchGlassSurface`: `NSVisualEffectView` `.hudWindow`/`.behindWindow`/`vibrantDark`, shaped via `maskImage`) — black over the content zone, melting to frosted glass at the bottom edge, with a luminous rim; toggle + bottom-darkness slider in Settings → Appearance → Open Notch. **Floating dock** (`NotchDockView` + shared `NotchTabCatalog`): tab capsules detached below the open panel plus a circular Settings button, hover handled by a notch⊍dock union hit-test shape, height reserved in `addShadowPadding` but excluded from the panel's layout proposal. Widgets integrated with the glass: scroll-edge fades converted from painted black gradients to transparency masks (`View+EdgeFade`), note editor lost its opaque canvas, Timer got the missing scroll-to-close suppression. New `SiriDotsSpinner` + shared `.siriGlow` modifier (replaces 5 copy-pasted blur+pulse recipes). Arrival banners: single-line label below the camera cutout, 80 pt expansion. **Isle mascot** (wave / sleeping / default / alert) on the welcome screen, Agents + Messages empty states, and as the menu bar template icon. Build 24. |
